...
> **NOTE**: With preloaded choices, use `search('your_csv')` with **one** argument so all rows load; this plug-in filters them. Do **not** add `'contains'` / `'matches'` and a second field. On Back, that filter often returns **zero** choices while the saved answer is still set, and SurveyCTO shows `Invalid choice value provided`. 
> See [Loom overview on fix (1.2.2)](https://www.loom.com/share/6d4aba5c4683480f99ec2702442e637a).
